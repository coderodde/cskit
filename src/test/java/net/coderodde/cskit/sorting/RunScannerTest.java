...
public class RunScannerTest {
    ...
    public void testScan() {
        ...
        assertEquals(1, (q = scanner.scan(array, 0, 6)).size());
        System.out.println(q);

        assertEquals(1, (q = scanner.scan(array, 6, 0)).size());
        System.out.println(q);

        array = new Integer[]{1, 2, 3, 4, 3, 2, 1};

        assertEquals(2, (q = scanner.scan(array, 0, 6)).size());
        System.out.println(q);

        assertEquals(2, (q = scanner.scan(array, 6, 0)).size());
        System.out.println(q);

        array = new Integer[]{1, 3, 2};

        assertEquals(2, (q = scanner.scan(array, 0, 2)).size());
        System.out.println(q);

        array = new Integer[]{1, 3};

        assertEquals(1, (q = scanner.scan(array, 0, 1)).size());
        System.out.println(q);

        assertEquals(1, (q = scanner.scan(array, 0, 0)).size());
        System.out.println(q);
